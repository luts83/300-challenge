...
⚠️ 응답 형식 규칙:
1. JSON만 출력하세요. 다른 텍스트는 절대 포함하지 마세요.
2. \`\`\`json, \`\`\` 등의 마크다운 문법을 사용하지 마세요.
3. 응답 시작과 끝에 아무것도 추가하지 마세요.
4. 순수한 JSON 객체만 반환하세요.
`;
...
const getAssignedTopicCriteria = (mode) => {
  if (mode === "mode_300") {
    return `
    // 300자 지정 주제 평가 기준 - 반드시 score와 feedback 필드를 모두 포함하세요
    "content": {
        "score": 0-100,
    ...
    "structure": {
        "score": 0-100,
        "feedback": "짧은 글의 구조와 흐름, 문법과 맞춤법에 대한 피드백 (제목과 본문의 일관성 포함)"
    },
    "impact": {
        "score": 0-100,
        "feedback": "독자에게 남기는 인상과 임팩트에 대한 피드백"
    }
    `;
  } else {
    return `
    // 1000자 지정 주제 평가 기준 - 반드시 score와 feedback 필드를 모두 포함하세요
    "content": {
        "score": 0-100,
    ...
    "originality": {
        "score": 0-100,
        "feedback": "주제에 대한 독창적인 관점과 해석에 대한 구체적인 피드백 (예시 포함)"
    },
    "consistency": {
        "score": 0-100,
        "feedback": "일관된 톤과 스타일에 대한 구체적인 피드백 (예시 포함)"
    },
    "expression": {
        "score": 0-100,
...
const getFreeTopicCriteria = (mode) => {
  if (mode === "mode_300") {
    return `
    // 300자 자유 주제 평가 기준 - 반드시 score와 feedback 필드를 모두 포함하세요
    "content": {
        "score": 0-100,
    ...
    "structure": {
        "score": 0-100,
        "feedback": "짧은 글의 구조와 흐름, 문법과 맞춤법에 대한 피드백"
    },
    "impact": {
        "score": 0-100,
        "feedback": "독자에게 남기는 인상과 임팩트에 대한 피드백"
    ...
    return `
    // 1000자 자유 주제 평가 기준 - 반드시 score와 feedback 필드를 모두 포함하세요
    "content": {
        "score": 0-100,
        "feedback": "주제 이해도, 내용의 깊이, 논리적 전개에 대한 심층적인 피드백"
    },
    "originality": {
        "score": 0-100,
